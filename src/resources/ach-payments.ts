...
export class AchPayments extends Base {
    async processAchPayment(
        ...
        // check type
        if (!req.type) {
          throw new Error("You must provide a 'type' value to process an ACH payment.  Accepted values are 'CREDIT' to payout an ACH payment or 'DEBIT' to draw payment from a customer account");
        }

        if (req.type.toUpperCase() !== "CREDIT" && req.type.toUpperCase() !== "DEBIT") {
          throw new Error("You must provide a 'type' value of 'CREDIT' to payout an ACH payment or 'DEBIT' to draw payment from a customer account");
        }

        // check payment detail
        if (!Object.keys(ach_detail).length && !req.token) {
          throw new Error("You must provide an 'ach_detail' object or a 'token' value to process an ACH payment");
        }
        if (ach_detail && req.token) {
          throw new Error("You must provide one of an 'ach_detail' object or a 'token' value to process an ACH payment.  Not both");
        }

        if (ach_detail.store_token && !customer_detail?.email) {
          throw new Error("You must provide a 'customer_detail.email' value to create and store an ACH payment token");
        }
        if (ach_detail.store_token && !ach_detail?.token_nickname) {
          throw new Error("You must provide an 'ach_detail.token_nickname' value to create and store an ACH payment token");
        }
          
        // check account type
        if (!ach_detail.account_kind || !ach_detail.account_type) {
          throw new Error("You must provide both an 'ach_detail.account_kind' and 'ach_detail.account_type' values to process a payment");
        }
        if (ach_detail.account_kind.toLocaleUpperCase() !== "PERSONAL" && ach_detail.account_kind.toLocaleUpperCase() !== "BUSINESS") {
          throw new Error("You must provide a 'ach_detail.account_kind' value of 'PERSONAL' or 'BUSINESS' to process a payment");
        }
        if (ach_detail.account_type.toLocaleUpperCase() !== "CHECKING" && ach_detail.account_type.toLocaleUpperCase() !== "SAVINGS") {
          throw new Error("You must provide a 'ach_detail.account_type' value of 'CHECKING' or 'SAVINGS' to process a payment");
        }
        ...
        const ep = req.type.toUpperCase() === 'DEBIT' ? 'payment/ach/debit' : 'payment/ach/credit';
        ...
        return this.request(`/${ep}`, {
          ...
          body: JSON.stringify({ transaction_data: {
            ...
            account_number: ach_detail?.account_number ? ach_detail?.account_number : req.token,
   ...
   *
   * @param {PaymentAchVoid} card - The ACH/Bank account payment void object.
  ...
  async processCreditCardVoid(
    ...
    return this.request(`/payment/ach/void`, {
      method: "POST",
   ...
   *
   * @param {PaymentAchRefund} card - The ACH/Bank account payment refund object.
  ...
  async processCreditCardRefund(
    ...
    return this.request(`/payment/ach/refund`, {
      method: "POST",
      body: JSON.stringify({ transaction_data: req }),
    });
  }
